// Modified by TaraWeb: portable expected paths on Windows.
...
test("maps safe sitemap filenames containing two dots", async () => {
  ...
  assert.deepEqual(loaded?.input, { path: resolve("/tmp/generated-sitemaps/sitemap..xml") });
});
...
test("maps encoded filenames without treating query or fragment text as a file path", async () => {
  ...
  assert.deepEqual(loaded?.input, { path: resolve("/tmp/generated-sitemaps/sitemap one.xml") });
