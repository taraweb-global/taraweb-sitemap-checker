...
export async function auditWeb(raw: unknown, checkStatus: boolean, signal: AbortSignal, fetcher = publicFetch) {
  const url = webUrl(raw).href;
  let downloaded = 0;
  const load = async (target: string) => {
    const result = await fetcher(target, signal);
    if (result.status < 200 || result.status >= 300) throw new Error(`Sitemap returned HTTP ${result.status}.`);
    downloaded += result.bytes.byteLength;
    if (downloaded > 8 * 1024 * 1024) throw new Error("Sitemap set exceeds the 8 MiB total limit.");
    return result;
  };
  const root = await load(url);
  const rows: { url: string; source: string; valid: boolean; status: number | null; statusNote: string }[] = [];
  ...
  for await (const event of validateSitemapSetEvents(root.bytes, {
    ...
    if (event.type === "sitemap:url") {
      if (rows.length >= 5000) throw new Error("This web check supports up to 5,000 URL entries. Use the CLI for larger sitemaps.");
      let valid = false;
      try { const parsed = new URL(event.loc ?? ""); valid = ["http:", "https:"].includes(parsed.protocol) && !parsed.username && !parsed.password && !parsed.hash && !/[\s\\]/u.test(event.loc ?? ""); } catch { /* Invalid entries remain visible. */ }
      rows.push({ url: event.loc ?? "(missing URL)", source: event.sourceId, valid, status: null, statusNote: "Not checked" });
    }
    if (event.type === "diagnostic") { diagnosticCount++; if (diagnostics.length < 200) diagnostics.push(event.diagnostic); }
    if (event.type === "set:summary") summary = event.summary;
  }
  if (checkStatus) {
    ...
    await Promise.all(Array.from({ length: 3 }, async (_, worker) => {
      for (let i = worker; i < sample.length; i += 3) {
        signal.throwIfAborted(); const row = sample[i]!;
        try { row.status = (await fetcher(row.url, signal, true)).status; row.statusNote = "HEAD response"; }
        catch { row.statusNote = "Unavailable or blocked"; }
      }
    }));
  }
