...
const server = createServer(async (req, res) => {
  ...
  if (req.headers.host !== new URL(origin).host) { json(403, { error: "Unrecognised host. Open the configured app URL." }); return; }
  const asset = assets.get(req.url ?? "");
  if (req.method === "GET" && asset) {
    try { res.setHeader("Content-Type", asset[1]!); res.end(await readFile(new URL(`../web/${asset[0]}`, import.meta.url))); }
    catch { json(500, { error: "Web assets unavailable." }); } return;
  }
  if (req.url !== "/api/validate" || req.method !== "POST") { json(404, { error: "Not found." }); return; }
  if (req.headers.origin !== origin || req.headers["content-type"] !== "application/json") { json(403, { error: "Use the form on the app page." }); return; }
  if (active >= 2) { res.setHeader("Retry-After", "10"); json(429, { error: "Two checks are already running. Please retry shortly." }); return; }
  active++;
  ...
  try {
    let body = "";
    for await (const chunk of req) { body += String(chunk); if (Buffer.byteLength(body) > 4096) throw new Error("Request is too large."); }
    const data: unknown = JSON.parse(body);
    if (!data || typeof data !== "object" || !("url" in data)) throw new Error("Enter a sitemap URL.");
